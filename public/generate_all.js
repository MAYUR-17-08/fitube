...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; -webkit-tap-highlight-color: transparent; }
    ...
    .top-nav { display: flex; align-items: center; justify-content: space-between; padding: 18px 20px 12px 20px; z-index: 10; }
    .nav-title { font-size: 26px; font-weight: 800; letter-spacing: -0.8px; }
    ...
    .banner-card { flex: 0 0 78vw; max-width: 320px; height: 180px; border-radius: 20px; position: relative; overflow: hidden; cursor: pointer; box-shadow: 0 12px 30px rgba(0,0,0,0.6); }
    ...
    .vertical-label { writing-mode: vertical-rl; transform: rotate(180deg); font-size: 16px; font-weight: 800; color: #fff; margin-right: 14px; text-align: center; }
    ...
    .track-row:active { background: var(--card-hover); }
    ...
    .track-actions-row { display: flex; align-items: center; gap: 12px; }

    /* Playlists UI */
    .create-playlist-btn { display: flex; align-items: center; gap: 12px; padding: 14px 16px; background: #1c1c24; border: 1px dashed var(--accent); border-radius: 16px; margin: 0 20px 14px 20px; cursor: pointer; color: #fff; }
    .playlist-folder { display: flex; align-items: center; justify-content: space-between; background: var(--card-bg); padding: 12px 16px; border-radius: 14px; margin: 0 20px 8px 20px; cursor: pointer; border: 1px solid #22222c; }

    /* Bottom Dock & Navbar */
    .mini-dock { position: fixed; bottom: 76px; left: 14px; right: 14px; background: rgba(26, 26, 32, 0.95); backdrop-filter: blur(20px); border: 1px solid #2a2a36; border-radius: 16px; padding: 7px 12px; display: none; align-items: center; justify-content: space-between; z-index: 90; cursor: pointer; box-shadow: 0 10px 30px rgba(0,0,0,0.8); }
    ...
    .bottom-navbar { position: fixed; bottom: 12px; left: 20px; right: 20px; height: 58px; background: rgba(18, 18, 22, 0.92); backdrop-filter: blur(25px); border: 1px solid rgba(255,255,255,0.08); border-radius: 30px; display: flex; align-items: center; justify-content: space-around; z-index: 100; box-shadow: 0 12px 35px rgba(0,0,0,0.9); }
    .nav-item { color: #828292; font-size: 18px; border: none; background: transparent; display: flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 20px; cursor: pointer; transition: 0.2s; }
    ...
    .player-artwork { width: 80vw; height: 80vw; max-width: 320px; max-height: 320px; border-radius: 24px; object-fit: cover; box-shadow: 0 20px 50px rgba(0,0,0,0.9); }
    ...
    /* Modals */
    .app-modal { position: fixed; inset: 0; background: rgba(9, 9, 11, 0.96); backdrop-filter: blur(25px); z-index: 350; display: none; flex-direction: column; padding: 20px; }
    .modal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; font-size: 20px; font-weight: 800; }
    .queue-item { display: flex; align-items: center; justify-content: space-between; background: var(--card-bg); border: 1px solid #22222a; padding: 10px 12px; border-radius: 14px; margin-bottom: 8px; }
    ...
    .order-btn:active { background: var(--accent); color: #000; }
...
<body>
  ...
  <audio id="bg-silent-anchor" loop preload="auto">
  ...
  <div class="main-view">
    ...
    <!-- Tab 3: Multiple Custom Playlists & Liked Songs -->
    <div class="page-tab" id="tab-library">
      ...
      <div class="section-header" style="padding: 14px 20px 10px 20px;"><span>Liked Songs ❤️</span></div>
      <div class="track-list" id="library-list"><p style="color:var(--text-muted); font-size:14px; text-align:center; margin-top:10px;">Songs you like will appear here.</p></div>
    </div>
  </div>

  <!-- Floating Mini Dock Player -->
  ...
  <div class="full-player-modal" id="full-player-modal">
    <div style="display:flex; justify-content:space-between; align-items:center;">
      ...
      <div style="font-size:11px; font-weight:800; color:var(--text-muted); letter-spacing:1px;">FITUBE NOW PLAYING</div>
    ...
    <div>
      <div style="display:flex; justify-content:space-between; align-items:center;">
        ...
        <div style="display:flex; gap:10px;">
          <button class="icon-btn" style="border:none; background:transparent; font-size:20px;" onclick="addCurrentSongToPlaylistModal()"><i class="fa-solid fa-folder-plus"></i></button>
          <button class="icon-btn" style="border:none; background:transparent; font-size:20px;" onclick="toggleFavoriteForSong(currentSong)"><i class="fa-regular fa-heart" id="full-like-btn"></i></button>
        </div>
      </div>
      ...
      <div class="player-controls-row">
        <button class="player-icon-btn" onclick="seekRelative(-10)"><i class="fa-solid fa-rotate-left"></i></button>
        <button class="player-big-btn" onclick="togglePlay()"><i class="fa-solid fa-play" id="full-play-icon"></i></button>
        <button class="player-icon-btn" onclick="playNextInQueue()"><i class="fa-solid fa-forward-step"></i></button>
      </div>
    </div>
  </div>

  <!-- Smart Vibe Infinite Queue Modal -->
  <div class="app-modal" id="queue-modal">
    <div class="modal-header"><span>Upcoming Vibe Queue</span><button class="icon-btn" onclick="closeQueueModal()"><i class="fa-solid fa-xmark"></i></button></div>
    <div style="flex:1; overflow-y:auto;" id="queue-items-container"></div>
  </div>

  <!-- Equalizer Modal -->
  <div class="app-modal" id="eq-modal">
    <div class="modal-header"><span>Sound Equalizer & Bass</span><button class="icon-btn" onclick="closeEqualizerModal()"><i class="fa-solid fa-xmark"></i></button></div>
  ...
  <div class="app-modal" id="playlist-view-modal">
    <div class="modal-header"><span id="opened-playlist-title">Playlist</span><button class="icon-btn" onclick="closePlaylistViewModal()"><i class="fa-solid fa-xmark"></i></button></div>
    <div style="flex:1; overflow-y:auto;" id="opened-playlist-songs"></div>
  </div>

  <!-- Add to Playlist Picker Modal -->
  <div class="app-modal" id="add-to-playlist-modal">
    <div class="modal-header"><span>Add To Playlist</span><button class="icon-btn" onclick="closeAddToPlaylistModal()"><i class="fa-solid fa-xmark"></i></button></div>
    <div style="display:flex; gap:8px; margin-bottom:16px;">
      <input type="text" id="new-modal-playlist-name" class="search-input-field" style="padding-left:14px;" placeholder="Create new playlist name..." />
      <button class="icon-btn" style="border-radius:14px; width:50px; background:var(--accent); color:#000;" onclick="createPlaylistFromPickerModal()"><i class="fa-solid fa-plus"></i></button>
    </div>
    <div style="flex:1; overflow-y:auto;" id="playlist-picker-list"></div>
  </div>
  ...
  <script>
    ...
    // Audio Equalizer Web Audio Pipeline
    ...
    const silentAudioAnchor = document.getElementById('bg-silent-anchor');

    function enableBackgroundMode() {
      if (silentAudioAnchor && silentAudioAnchor.paused) silentAudioAnchor.play().catch(()=>{});
    }
    ...
    async function performSearch(q) {
      ...
      try {
        ...
        container.innerHTML = songs.map(s => {
          ...
          return '<div class="track-row" onclick=\'playSongDirect(' + JSON.stringify(s).replace(/'/g, "&apos;") + ')\'><div class="track-info-box"><img src="' + s.thumbnail + '" /><div class="track-texts"><div class="track-row-title">' + s.title + '</div><div class="track-row-artist">' + s.artist + ' ' + (s.duration ? '• ' + s.duration : '') + '</div></div></div><div class="track-actions-row"><i class="' + (isFav ? 'fa-solid' : 'fa-regular') + ' fa-heart" style="color:' + (isFav ? '#ff4d4d' : '#8b8b99') + '; font-size:16px; padding:6px;" onclick=\'event.stopPropagation(); toggleFavoriteForSong(' + JSON.stringify(s).replace(/'/g, "&apos;") + ')\'></i><i class="fa-solid fa-folder-plus" style="color:#fff; font-size:15px; padding:6px;" onclick=\'event.stopPropagation(); openPickerForSpecificSong(' + JSON.stringify(s).replace(/'/g, "&apos;") + ')\'></i><i class="fa-solid fa-play" style="color:var(--accent); font-size:14px;"></i></div></div>';
        }).join('');
    ...
    // Playlist Creation & Detailed View
    function createNewPlaylist() {
      const name = prompt("Enter Playlist Name (e.g. Gym Vibes, Dance Party):");
    ...
    function openPlaylistViewModal(plId) {
      const pl = userPlaylists.find(p => p.id === plId);
      if (!pl) return;
      document.getElementById('opened-playlist-title').innerText = pl.name;
      const songsContainer = document.getElementById('opened-playlist-songs');
      if (!pl.tracks.length) {
        songsContainer.innerHTML = '<p style="color:var(--text-muted); font-size:14px; text-align:center; margin-top:40px;">No songs in this playlist yet. Add songs using the folder icon.</p>';
      } else {
        songsContainer.innerHTML = pl.tracks.map((s, idx) => '<div class="track-row" onclick=\'playSongDirect(' + JSON.stringify(s).replace(/'/g, "&apos;") + ')\'><div class="track-info-box"><img src="' + s.thumbnail + '" /><div class="track-texts"><div class="track-row-title">' + s.title + '</div><div class="track-row-artist">' + s.artist + '</div></div></div><button class="order-btn" style="color:#ff4d4d;" onclick=\'event.stopPropagation(); removeSongFromPlaylist("' + plId + '", ' + idx + ')\'><i class="fa-solid fa-trash"></i></button></div>').join('');
      }
      document.getElementById('playlist-view-modal').style.display = 'flex';
    }

    function closePlaylistViewModal() { document.getElementById('playlist-view-modal').style.display = 'none'; }

    function removeSongFromPlaylist(plId, songIdx) {
      const pl = userPlaylists.find(p => p.id === plId);
      if (pl) {
        pl.tracks.splice(songIdx, 1);
        localStorage.setItem('fitube_custom_playlists', JSON.stringify(userPlaylists));
        openPlaylistViewModal(plId);
        renderLibrary();
      }
    }

    let songPendingAddition = null;

    function addCurrentSongToPlaylistModal() {
      if (!currentSong) return;
      openPickerForSpecificSong(currentSong);
    }

    function openPickerForSpecificSong(song) {
      songPendingAddition = song;
    ...
    function closeAddToPlaylistModal() {
      ...
      songPendingAddition = null;
    }

    function renderPlaylistPickerList() {
      const container = document.getElementById('playlist-picker-list');
      if (!userPlaylists.length) {
        container.innerHTML = '<p style="color:var(--text-muted); font-size:14px; text-align:center; margin-top:20px;">No playlists found. Type a name above to create one!</p>';
        return;
      }
      container.innerHTML = userPlaylists.map((p, idx) => '<div class="playlist-folder" onclick="addSongToPlaylistIndex(' + idx + ')"><div style="display:flex; align-items:center; gap:12px;"><i class="fa-solid fa-music" style="color:var(--accent);"></i><div><div style="font-weight:700; font-size:14px;">' + p.name + '</div><div style="font-size:11px; color:var(--text-muted);">' + p.tracks.length + ' Songs</div></div></div><i class="fa-solid fa-plus" style="color:var(--accent);"></i></div>').join('');
    }

    function createPlaylistFromPickerModal() {
      const input = document.getElementById('new-modal-playlist-name');
      const name = input.value.trim();
      if (!name) return;
      const newPl = { id: 'pl_' + Date.now(), name: name, tracks: [] };
      if (songPendingAddition) newPl.tracks.push(songPendingAddition);
      userPlaylists.unshift(newPl);
      localStorage.setItem('fitube_custom_playlists', JSON.stringify(userPlaylists));
      input.value = '';
      closeAddToPlaylistModal();
      renderLibrary();
      alert('Playlist "' + name + '" created & song added!');
    }

    function addSongToPlaylistIndex(idx) {
      if (!songPendingAddition || !userPlaylists[idx]) return;
      if (!userPlaylists[idx].tracks.some(t => t.id === songPendingAddition.id)) {
        userPlaylists[idx].tracks.unshift(songPendingAddition);
        localStorage.setItem('fitube_custom_playlists', JSON.stringify(userPlaylists));
        alert('Added to "' + userPlaylists[idx].name + '"!');
      } else {
        alert('Song is already in "' + userPlaylists[idx].name + '"');
      }
      closeAddToPlaylistModal();
      renderLibrary();
    }
    ...
    function togglePlay() {
      if (ytPlayer && ytPlayer.getPlayerState) {
        if (ytPlayer.getPlayerState() === YT.PlayerState.PLAYING) {
          ytPlayer.pauseVideo();
          if (silentAudioAnchor) silentAudioAnchor.pause();
        } else {
          ytPlayer.playVideo();
          enableBackgroundMode();
        }
...
console.log('100% Complete Fitube Master UI successfully generated!');
